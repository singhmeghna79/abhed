// Package store keeps the enterprise edition's tables beside the Community
// event store: same database, same tenant setting, one thing to back up.
package store
...
// it belongs in the database — and it belongs to this edition, because only a
// deployment that admits strangers has anyone to record.
//
// A wrapper rather than methods on the Community store: Go does not let one
// package add methods to another's type, and that limit is doing useful work
// here. The Community store exposes its pool and its tenant and nothing about
// grants; every table below is this package's to create, version and read.
type Access struct {
	pool   *pgxpool.Pool
	tenant string

	// Applied on first use, once, the way the Community store applies its
	// accounts table: the operations below call MigrateAccess defensively,
	// and DDL on every request would take locks against tables that exist.
	once sync.Once
	err  error
}

// NewAccess keeps access records in the database behind a Community store.
func NewAccess(pg *cestore.Postgres) *Access {
	return &Access{pool: pg.Pool(), tenant: pg.Tenant()}
}

// MigrateAccess applies the access schema. Idempotent, with a version table
// of its own so it never writes the event store's.
func (a *Access) MigrateAccess(ctx context.Context) error {
	a.once.Do(func() {
		if _, err := a.pool.Exec(ctx, accessSQL); err != nil {
			a.err = fmt.Errorf("apply access schema: %w", err)
		}
	})
	return a.err
}
...
func (a *Access) RecordRequest(ctx context.Context, g Grant) (Grant, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return Grant{}, err
	}
	if g.ID == "" {
		g.ID = newGrantID()
	...
	_, err := a.pool.Exec(ctx, `
		...
		g.ID, a.tenant, strings.ToLower(g.Email), g.Name, g.Company, g.UseCase,
		g.Status, g.RequestedAt, g.ScreenScore, g.ScreenNote)
	if err != nil {
		return Grant{}, fmt.Errorf("record access request: %w", err)
	}
	return g, a.logAccess(ctx, g.ID, "system", "requested", g.ScreenNote)
}

// GrantAccess marks a request granted and attaches the invite issued for it.
func (a *Access) GrantAccess(ctx context.Context, id, by, code string, expires time.Time) error {
	if err := a.MigrateAccess(ctx); err != nil {
		return err
	}
	now := time.Now().UTC()
...
func (a *Access) Redeemed(ctx context.Context, code, username string) error {
	...
	return a.logAccess(ctx, id, username, "redeemed", "account created")
}

// Revoke ends access and records why. The clause is what the person is told;
// the note is for whoever reads this later.
func (a *Access) Revoke(ctx context.Context, id, by, clause, note string) (Grant, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return Grant{}, err
	}
	now := time.Now().UTC()
	...
	if err := a.logAccess(ctx, id, by, "revoked", clause+": "+note); err != nil {
		return Grant{}, err
	}
	return a.GrantByID(ctx, id)
}

// GrantByID reads one grant.
func (a *Access) GrantByID(ctx context.Context, id string) (Grant, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return Grant{}, err
	}
	rows, err := a.pool.Query(ctx, accessSelect+` WHERE id = $1`, id)
	if err != nil {
...
func (a *Access) GrantByUsername(ctx context.Context, username string) (Grant, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return Grant{}, err
	}
...
func (a *Access) Grants(ctx context.Context) ([]Grant, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return nil, err
	}
	rows, err := a.pool.Query(ctx, accessSelect+` ORDER BY requested_at DESC`)
	if err != nil {
...
func (a *Access) AccessHistory(ctx context.Context, id string) ([]AccessEvent, error) {
	if err := a.MigrateAccess(ctx); err != nil {
		return nil, err
	}
	rows, err := a.pool.Query(ctx,
...
func (a *Access) logAccess(ctx context.Context, grantID, actor, action, detail string) error {
	_, err := a.pool.Exec(ctx,
		`INSERT INTO access_events (tenant_id, grant_id, actor, action, detail)
		 VALUES ($1, $2, $3, $4, $5)`,
		a.tenant, grantID, actor, action, detail)
	if err != nil {
