# Titan vs. aider vs. a bare agent loop — a reproducible local benchmark
...
```
env -u GOROOT go build -o <scratch>/bin/titan ./cmd/titan
```
...
### 6. Titan config used for every task
...
```
titan -C <workspace> -mode accept-edits -allow 'bash(python3 -m pytest*)' \
...
default CLI ergonomics, but it is not a fair comparison to Titan and the bare
...
titan, then aider, then bare, and was still on titan when the invocation was
...
workspaces, the exercism clone, the built `titan` binary, the Python venv, and
