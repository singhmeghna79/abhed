# Results — Titan vs. aider vs. a bare agent loop
...
**Titan commit:** `de52025` (built as `cmd/titan`; see the naming note in
`README.md` — this branch has since renamed the project to "Abhed").
...
|---|---|---|---|---|---|
| **titan** | **24/24 (100%)** | 137.6 | 59,607 | 10.0 | 0/24 |
...
**Titan wins on correctness, by a real margin, and it is not close.** 24/24
...
**Titan does not win on speed or token economy, and pretending otherwise
...
of it is Titan's own tool-call overhead (each `read`/`write`/`bash` round
trip through the sandboxed process costs real wall time on top of the model's
own latency). The honest reading: Titan trades tokens and time for a
...
aider had the same information as Titan and the bare agent. Its five failures
...
exercises even with almost no scaffolding. Titan's edge over bare (24 vs. 22)
...
**`tournament` is the outlier for cost.** Titan took 577.4s on it — by far
its longest run — while aider (218.1s, and still failed it) and bare (267.6s,
passed) were both faster. That one task alone pulls Titan's mean wall time up
substantially; without it, Titan's mean would be noticeably closer to the
other two systems. Look at the per-task table before generalizing "Titan is
slower" to every task — on the small, simple exercises (`leap`, `hamming`,
`raindrops`, `pangram`) Titan's wall time is close to or better than aider's.

## Per-system detail

| System | Pass | Fail | Mean wall time (s) | Median wall time (s) | Mean tokens | Mean turns |
|---|---|---|---|---|---|---|
| titan | 24 | 0 | 137.6 | 113.8 | 59,607 | 10.0 |
...
| Task | titan | aider | bare | titan time(s) | aider time(s) | bare time(s) |
|---|---|---|---|---|---|---|
...
aider phase began; titan was still running its 24 tasks), this invocation
...
a repo-aware agent by default, unlike Titan and the bare agent, which both
