// Video 3 — Installing Titan, from a laptop to an air-gapped enclave.
window.VIDEOS['03-install'] = () => {
  ...
  return [
    { beat: 'b1', build(s) {
      Parts.logo(s, { x: 96, y: 96, text: 'Titan', mark: 'titan', prod: 'Install', at: 0.2, size: 80 });
      ...
      Parts.foot(s, 'ZYBUU · TITAN · SERIES 3 OF 4', 2.0);
    } },
    ...
    { beat: 'b2', build(s) {
      ...
      Parts.diagram(s, { nodes: [
        { id: 't', x: 700, y: 480, w: 320, h: 120, html: 'titan<small>one static binary</small>', cls: 'hot', at: 2.2 },
    ...
    // b4: requirements
    { beat: 'b4', build(s) {
      title(s, 'What you need', 'Sized by the model, <span class="hl">not by Titan.</span>');
    ...
    { beat: 'b5', build(s) {
      ...
      Parts.terminal(s, { x: 96, y: 440, w: 1200, h: 520, title: '~ — zsh', at: 1.2, size: 25, lines: [
        ['cmd', '$ git clone <your mirror>/titan && cd titan', 1.6, true],
        ['cmd', '$ go build -o ~/.local/bin/titan ./cmd/titan', 3.4, true],
        ['ok', '  static binary · CGO_ENABLED=0 · pgx, x/crypto, x/term', 5.0],
        ['cmd', '$ ollama pull gemma4:26b', 6.0, true],
        ['cmd', '$ titan init', 7.6, true],
        ['dim', '  wrote .titan/config.json · detected http://localhost:11434/v1', 8.4],
        ['cmd', '$ titan doctor', 9.4, true],
      ] });
    ...
    { beat: 'b6', build(s) {
      title(s, 'titan doctor', 'Ten seconds that <span class="hl">save a run.</span>');
      Parts.terminal(s, { x: 96, y: 400, w: 1240, h: 620, title: '~/work — titan doctor', at: 1.0, size: 22, lines: [
        ['cmd', '$ titan doctor', 1.2, true],
    ...
    { beat: 'b7', build(s) {
      ...
      Parts.terminal(s, { x: 96, y: 440, w: 900, h: 430, title: 'deploy — run.sh', at: 1.0, size: 23, lines: [
        ['cmd', '$ ./deploy/run.sh serve', 1.3, true], ['dim', 'creating volume titan-workspace', 2.6], ['dim', 'creating volume titan-state', 2.8], ['dim', 'creating volume titan-db-data', 3.0],
        ['dim', 'starting titan-db … ready', 3.6], ['ok', 'provisioned role titan_app (NOSUPERUSER NOBYPASSRLS)', 4.6], ['ok', 'titan listening on 127.0.0.1:8080', 5.6],
      ] });
      Parts.diagram(s, { nodes: [
        { id: 'net', x: 1100, y: 420, w: 720, h: 460, html: '', at: 6.0 },
        { id: 't', x: 1180, y: 520, w: 260, h: 110, html: 'titan<small>127.0.0.1:8080</small>', cls: 'hot', at: 6.4 },
        { id: 'db', x: 1500, y: 520, w: 260, h: 110, html: 'titan-db<small>no published port</small>', at: 6.9 },
      ...
      const lab = Parts.pill(s, 'network: titan-net', { x: 1120, y: 396, at: 6.2 });
      Parts.check(s, 'Titan is the only client of the database', { x: 96, y: 920, at: 9.6 });
    } },
    ...
    { beat: 'b9', build(s) {
      ...
      Parts.diagram(s, { nodes: [
        { id: 'adm', x: 200, y: 520, w: 380, h: 120, html: 'titan_admin<small>superuser · provisions only</small>', at: 1.8 },
        { id: 'app', x: 760, y: 520, w: 380, h: 120, html: 'titan_app<small>NOSUPERUSER · NOBYPASSRLS · owns the tables</small>', cls: 'ok', at: 3.4 },
      ...
      Parts.check(s, 'Titan refuses to start as a superuser or BYPASSRLS role', { x: 200, y: 760, at: 7.4 });
    ...
    { beat: 'b10', build(s) {
      ...
      Parts.terminal(s, { x: 96, y: 400, w: 1180, h: 620, title: '/etc/titan/config.json', at: 1.0, size: 21, lines: [
        ['cmd', '{', 1.2],
        ['tool', '  "model":   { "default": "onprem", "providers": { "onprem": {', 2.0], ['dim', '                 "type": "openai-compatible", "base_url": "http://vllm.internal:8000/v1",', 2.2],
        ['dim', '                 "model": "Qwen/Qwen3-32B", "api_key_env": "TITAN_API_KEY" } } },', 2.4],
        ...
        ['cmd', '}', 8.4],
        ['ok', '# TITAN_DATABASE_URL and TITAN_API_KEY come from the environment', 9.8],
      ] });
    ...
    { beat: 'b11', build(s) {
      ...
      Parts.card(s, { x: 96, y: 460, w: 700, h: 300, icon: ICONS.user, title: 'Local accounts', text: 'Issued by an administrator: <span class="mono">titan user add</span>. Invite-only registration, never open.', at: 1.6 });
    ...
    { beat: 'b12', build(s) {
      ...
      Parts.terminal(s, { x: 96, y: 440, w: 1000, h: 480, title: 'connected build host', at: 1.0, size: 23, lines: [
        ...
        ['dim', 'bin/     titan · titan-bench  ×  linux/amd64 linux/arm64 darwin/arm64 darwin/amd64', 3.6],
        ...
        ['ok', 'titan-0.1.0.tar.gz  +  .sha256  +  .sig', 7.6],
      ] });
    ...
    { beat: 'b13', build(s) {
      ...
      Parts.terminal(s, { x: 96, y: 440, w: 1000, h: 480, title: 'air-gapped target — no route out', at: 1.0, size: 23, lines: [
        ['cmd', '$ ./scripts/verify-bundle.sh titan-0.1.0.tar.gz release.pub', 1.3, true],
        ...
        ['cmd', '$ tar xzf titan-0.1.0.tar.gz && ./install.sh', 9.0, true], ['dim', 'installed to /opt/titan', 10.6],
      ] });
    ...
    { beat: 'b14', build(s) {
      ...
      Parts.diagram(s, { nodes: [
        ...
        { id: 't', x: 480, y: 520, w: 300, h: 110, html: 'titan<small>/opt/titan</small>', cls: 'hot', at: 1.6 },
    ...
    { beat: 'b15', build(s) {
      ...
      const rows = [['titan user add <name>', 'issue an account, or connect OIDC', 1.6], ['"telemetry": { "enabled": true, "endpoint": "http://otel:4318" }', 'traces into your collector', 3.4], ['"schedules": [{ "cron": "@hourly", … }]', 'recurring work, on the record', 5.0], ['replace one binary', 'the schema migrates itself on start; every migration is idempotent', 6.8]];
    ...
    { beat: 'b16', build(s) {
      ...
      const u = Parts.pill(s, 'titan.zybuu.com/docs', { x: 790, y: 720, at: 5.2 }); u.style.fontSize = '30px';
    } },
  ];
};
